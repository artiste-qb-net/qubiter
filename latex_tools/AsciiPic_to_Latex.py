...
class AsciiPic_to_Latex:
    ...
    this class inserts an intermediate translation step that we call a Tiny
    ascii picture. A Tiny picture is a simplified, abridged version of a
    Qubiter "native" Picture file. In Qubiter native pictures, each gate is
    made up of multiple bonds, and each bond is 4 characters long. A Tiny
    picture uses 1 character per bond instead of 4, usually retaining only
    the first character of each bond of the counterpart Qubiter native
    picture. Hence, a Tiny picture is exactly 1/4 as long as its Qubiter
    native picture counterpart. Qubiter native pictures and Tiny pictures
    can both be machine generated by Qubiter or written by hand. Because
    they are 1/4 as long, Tiny pictures are easier to write by hand than
    Qubiter native pictures.

    Here is a quantum circuit with 7 qubits and 9 gates expressed in Qubiter
    native picture format and in Tiny picture format. In both pictures,
    ...
    As you can see, in the Tiny pic, the white spaces and hyphens are
    ...
    In translating a Tiny picture to Latex, this class will try to interpret
    each character of a Tiny pic by using the "kept" list above. If you
    insert a character that it can't interpret, like for example an 'A',
    it will translate that character to a Latex one qubit box with an 'A'
    inside it. If a gate (=time=tiny pic row) has at least one '?' in the
    tiny pic, it will be drawn in the Latex with inter-qubit wiring. For
    example, a tiny row |AA|?| will be drawn in the Latex with a box around
    each A and a box around the ? and a wire connecting all 3 boxes.

    In Qubiter native pics and Tiny pics, time points down, the way we
    ...
    gb_char_arr : np.ndarray
        ...
        splitting a Tiny picture into a character array, then we make some
    ...
    init_states : list[str]
        ...
        qubit order.
    reverse_bits : bool
        Set to True iff you want qubit order in Tiny pic to be reversed in
        Latex pic.
    reverse_gates : bool
        Set to True iff you want gate order in Tiny pic to be reversed in
        Latex pic.
    time_dir : str
    ...
    def __init__(self, reverse_bits=True, reverse_gates=True):
        ...
        # these change with each tiny_word_list considered
    ...
    def process_tiny_word_list(self, tiny_word_list, init_states):
        """
        Internal function that uses info in the input tiny_word_list to fill
        certain attributes of the class.

        Parameters
        ----------
        tiny_word_list : list[str]
        init_states : list[str]
        ...
        word_list = cp.copy(tiny_word_list)
    ...
    def get_ckt_latex(self, tiny_word_list, init_states=None):
        """
        ...
        self.process_tiny_word_list(tiny_word_list, init_states)

        latex_str = "\\begin{equation}\n\\begin{array}{c}\n"
        latex_str += "\\Qcircuit @C=2em @R=.4em {\n"

        num_bits = len(tiny_word_list[0])
        num_gates = len(tiny_word_list) + 2
