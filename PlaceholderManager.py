...
class PlaceholderManager:
    ...
    class owns a dictionary var_nums_to_degs that is used to store values of
    gate variables.
    ...
    var_nums_to_degs : dict[int, float]
    ...
    def __init__(self, no_vars=False, eval_all_vars=True,
        ...
        self.var_nums_to_degs = var_nums_to_degs
        self.num_vars = 0
        self.cur_var_num = -1
    ...
    def degs_str_to_rads(self, degs_str):
        ...
        variable in the self.var_nums_to_degs dictionary. If it finds a
        ...
        if degs_str[0] == '#':
            ...
            if self.var_nums_to_degs:
                key_exists = var_num in self.var_nums_to_degs.keys()
                if key_exists:
                    return self.var_nums_to_degs[var_num]*np.pi/180
                else:  # key doesn't exist
                    if self.eval_all_vars:
                        assert False, 'no value for variable #' + str(var_num)
                    else:
                        return degs_str
            else:  # no self.var_nums_to_degs dict
                if self.eval_all_vars:
                    assert False, 'no self.var_nums_to_degs dict'
                else:
                    return degs_str
        else:
...
if __name__ == "__main__":
    ...
    def main():
        ...
        vman = PlaceholderManager(
            var_nums_to_degs={0: 30, 1: 60, 2: 45})
