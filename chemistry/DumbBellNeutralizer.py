...
class DumbBellNeutralizer:
    ...
    one uses Jordan Wigner (JW) strings to model the fermionic degrees of
    freedom. These JW strings lead to clusters of adjacent JW dumbbells.

    The square of a dumbbell equals one. The square of a CNOT does too,
    and Microsoft has a patent (by Wecker/Hastings) that basically boils
    down to: We claim that we are the first to replace the square of a CNOT
    by one. I would like to forewarn the user of this class. It's possible
    that MS will claim in the future that this class infringes on their
    CNOT^2 = 1 patent.
