...
class ChipCouplingsFitter:
    ...
    graph, call it GP=(V_GP, E_GP) for Graph_Physical, from the c_to_t
    ...
    all allowed** by the chip constraint c_to_t. In that sense, the map
    ...
    **except, some CNots may have to be reversed (control and target
    ...
    def __init__(self, file_prefix, num_bits, c_to_t, verbose=False):
        ...
        num_bits : int
            ...
            `file_prefix`. IMP: We assume that c_to_t refers to a chip with
        ...
        c_to_t : tuple[tuple[int, int]]
            Tuple of tuples (c, t), where a CNot with control bit position c
            and target bit position t is physically allowed by the chip
            described by c_to_t.
        verbose : bool
        ...
        self.bit_map = ChipCouplingsFitter.get_bit_map_from_c_to_t(
            num_bits, old_cnots, c_to_t, verbose)
    ...
    def get_bit_map_from_c_to_t(num_bits, old_cnots, c_to_t, verbose=False):
        ...
        English file for a circuit with num_bits qubits and `c_to_t`
        ...
        CNots which are all allowed** by the chip couplings constraint c_to_t.
